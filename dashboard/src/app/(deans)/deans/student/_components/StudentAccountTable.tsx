...
export default function StudentAccountTable() {
  ...
  const columns: ColumnDef<UserProps>[] = [
    ...
    },
    {
      ...
      header: 'Group',
      cell: (cell) =>
        cell.row.original?.groups?.map((name) => name.name).join(', '),
      size: 15,
    },
    {
      header: 'Action',
      cell: (cell) => (
        <div className='flex gap-3'>
          <IconLink
            icon={Edit2Icon}
            href={`/deans/student/${cell.row.original.id}`}
          />
          <IconButton
            icon={Trash2Icon}
            onClick={() =>
  ...
  const url = buildPaginatedTableURL({
    ...
    additionalParam: {
      ...
      filter_value: [tableState.globalFilter ?? null, 1],
      related_tables: ['faculties', 'groups'],
    },
  });
