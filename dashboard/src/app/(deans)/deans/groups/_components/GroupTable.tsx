...
import EditGroupModal from '@/app/(deans)/deans/groups/_components/EditGroupModal';
import GroupAdminForm from '@/app/(deans)/deans/groups/_components/GroupForm';
...
export default function GroupAdminTable({
  ...
  const deleteNews = async ({ id }: { id: string }) => {
    const response = await api.delete('/master/groups', {
      data: {
        id: id,
      },
    });
  ...
  return (
    <>
      <section className='my-5 w-1/3'>
        <GroupAdminForm refetch={refetchFaculty} faculty_id={faculty_id} />
      </section>
      <ServerTable
        meta={newsData?.meta}
