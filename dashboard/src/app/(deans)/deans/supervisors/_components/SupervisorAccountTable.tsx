...
export default function SupervisorAccountTable() {
    ...
    },
    {
      ...
      cell: (cell) => (
        <ul className='grid grid-cols-2 gap-3'>
          {cell.row.original.keywords.map((keyword) => (
            <Keywords key={keyword.name} keywords={keyword.name} />
          ))}
        </ul>
      ),
      size: 35,
    },
    {
      ...
      cell: (cell) => (
        <div className='flex gap-3'>
          <IconLink
            icon={Edit2Icon}
            href={`/deans/supervisors/${cell.row.original.id}`}
          />
          <IconButton
            icon={Trash2Icon}
            onClick={() =>
  ...
  const url = buildPaginatedTableURL({
    ...
    additionalParam: {
      ...
      filter_value: [tableState.globalFilter ?? null, 2],
